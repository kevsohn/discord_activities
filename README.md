...
1) Register the game in shared/game_reg.json following the outlined schema

2) Add a game UI class in client/src/games/ + register it in client/src/App.jsx 

3) Add a GameEngine child in server/src/engines/ to define game logic + register it in server/src/depends/engine_reg.py following the outlined schema

4) Add any data provider modules in server/src/providers/


project/
├── shared/                     # Shared between front & backend
│   └── game_reg.json           # Game registry & specifications
|
...
│   ├── vite.config.js
│   └── src/
│       ├── main.jsx            # App entry point & render
│       ├── App.jsx             # App setup and UI
|       |
│       ├── services/           
│       │   ├── auth.js         # Discord OAuth2 flow
│       │   └── sessions.js     # SessionController (backend interface)
|       |
